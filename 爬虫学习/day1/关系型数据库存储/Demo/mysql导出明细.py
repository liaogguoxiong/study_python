...
def output_data(ip,db_name,port,shuihao,company_name):
    ...
    for i in shuihao:
        ...
        #print(sql)
        os.mkdir('C:/Users/xiao/Desktop/数据导出/{company_name}/{dicname}'.format(company_name=company_name,dicname=name))
        f = open('C:/Users/xiao/Desktop/数据导出/{company_name}/{dicname}/{filename}.csv'.format(company_name=company_name,dicname=name,filename=name), 'a', newline='', encoding='utf-8')
        writer = csv.writer(f)
        writer.writerow(
